...
import { FileImage, FileType, FileCode, FileText, Info } from 'lucide-react';
...
const FSMCanvas: React.FC<FSMCanvasProps> = ({ 
  ...
  const handleExport = useCallback((format: 'png' | 'svg' | 'pdf' | 'txt') => {
    ...
    if (flowElement) {
        ...
        if (format === 'png') {
            toPng(flowElement, options).then((dataUrl) => {
                const link = document.createElement('a');
            ...
            toSvg(flowElement, options).then((dataUrl) => {
                const link = document.createElement('a');
            ...
            toPng(flowElement, { ...options, pixelRatio: 2 }).then((dataUrl) => {
  ...
  return (
    <div className={`h-full w-full bg-[#f8f9fa] relative font-serif-academic group ${isConnectMode ? 'cursor-crosshair' : ''}`}>
      ...
      >
        ...
        <MiniMap 
            nodeColor={(n) => n.type === 'group' ? '#eff6ff' : '#fff'}
            nodeStrokeWidth={3} 
            zoomable 
            pannable 
        ...
        <Panel position="top-right" className="m-4 flex flex-col gap-3 items-end">
            <div className="flex flex-col items-end gap-1">
                ...
                <div className="flex gap-2">
                    <button onClick={() => handleExport('png')} className={btnClass} title="Download PNG"><FileImage size={14} /></button>
                    <button onClick={() => handleExport('svg')} className={btnClass} title="Download SVG"><FileCode size={14} /></button>
                    <button onClick={() => handleExport('pdf')} className={btnClass} title="Download PDF"><FileType size={14} /></button>
                    <button onClick={() => handleExport('txt')} className={btnClass} title="Download Report (TXT)"><FileText size={14} /></button>
                </div>
            </div>
